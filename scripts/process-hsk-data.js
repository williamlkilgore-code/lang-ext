...
// Filter for HSK 1-5 words (old system to match user's requested levels)
const hsk12345Words = Object.values(hskData).filter(entry => {
  if (!entry.level) return false;
  return entry.level.some(level =>
    level === 'old-1' || level === 'old-2' || level === 'old-3' || level === 'old-4' || level === 'old-5'
  );
});

console.log(`Found ${hsk12345Words.length} words in HSK 1-5`);

// Group by radical for our dictionary structure
const radicalGroups = {};
const wordsList = [];

hsk12345Words.forEach((entry, index) => {
...
const chineseDictionary = {
  ...
  language: 'chinese',
  variant: 'simplified',
  description: 'HSK 1-5 vocabulary with radical decomposition',
  levels: 'HSK 1-5 (old system)',
