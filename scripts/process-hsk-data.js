...
// Filter for HSK 1-6 words (old system - complete HSK vocabulary)
const hsk123456Words = Object.values(hskData).filter(entry => {
  if (!entry.level) return false;
  return entry.level.some(level =>
    level === 'old-1' || level === 'old-2' || level === 'old-3' ||
    level === 'old-4' || level === 'old-5' || level === 'old-6'
  );
});

console.log(`Found ${hsk123456Words.length} words in HSK 1-6 (Complete HSK)`);

// Group by radical for our dictionary structure
const radicalGroups = {};
const wordsList = [];

hsk123456Words.forEach((entry, index) => {
...
const chineseDictionary = {
  ...
  language: 'chinese',
  variant: 'simplified',
  description: 'HSK 1-6 vocabulary with radical decomposition (Complete HSK)',
  levels: 'HSK 1-6 (old system)',
